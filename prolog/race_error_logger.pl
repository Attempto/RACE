...
@author Tobias Kuhn
@author Kaarel Kaljurand (replaced assert/retract with b_setval/b_getval)
@author Norbert E. Fuchs (replaced b_setval/b_getval with nb_setval/nb_getval)
@version 2008-08-31
*/
...
%
% This predicate succeeds even if there are no
% messages of this type and select/3 fails.

clear_messages(Type) :-
	nb_getval(message, Messages),
	select(message(_, Type, _, _, _), Messages, RemainingMessages),
	nb_setval(message, RemainingMessages),
	!.
...
%
% This predicate succeeds even if there are no
% error messages and select/3 fails.

clear_error_messages :-
	nb_getval(message, Messages),
	select(message(error, _, _, _, _), Messages, RemainingMessages),
	nb_setval(message, RemainingMessages),
	!.
...
%
% This predicate succeeds even if there are no
% warning messages and select/3 fails.

clear_warning_messages :-
	nb_getval(message, Messages),
	select(message(warning, _, _, _, _), Messages, RemainingMessages),
	nb_setval(message, RemainingMessages),
	!.
...
assert_message(Importance, Type, SentenceID-TokenID, Subject, Description) :-
	...
	nb_getval(message, Messages),
	nb_setval(message, [message(Importance, Type, SentenceID-TokenID, Subject, Description) | Messages]).
